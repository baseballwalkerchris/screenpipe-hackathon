...
export interface LiveMeetingData {
  ...
  visionChunks: VisionChunk[]; // Add vision chunks
  mergedVisionChunks: VisionChunk[]; // Add merged vision chunks
...
interface MeetingContextType {
  ...
  onNewVisionChunk: (chunk: VisionChunk) => Promise<void>;
...
export function MeetingProvider({ children }: { children: ReactNode }) {
  ...
  const [improvingChunks, setImprovingChunks] = useState<
    Record<number, boolean>
  >({});
  const [recentlyImproved, setRecentlyImproved] = useState<
    Record<number, boolean>
  >({});
  ...
  const { handleNewChunk, handleNewVisionChunk } = createHandleNewChunk({
    setData,
    setImprovingChunks,
    setRecentlyImproved,
    updateStore,
  ...
  const value = useMemo(
    () => ({
      ...
      segments: (data?.chunks || []).map((chunk) => ({
        ...
        speaker:
          data?.speakerMappings[chunk.speaker || "speaker_0"] ||
          chunk.speaker ||
          "speaker_0",
      })),
      setSegments: async () => {},
      ...
      onNewVisionChunk: handleNewVisionChunk,
      improvingChunks,
      setImprovingChunks,
      recentlyImproved,
      setRecentlyImproved,
...
export async function deleteArchivedMeeting(startTime: string): Promise<void> {
  try {
    ...
    await meetingStore.iterate<LiveMeetingData, void>(
      (value: LiveMeetingData, key: string) => {
...
export async function updateArchivedMeeting(
  id: string,
  update: Partial<LiveMeetingData>
) {
  try {
