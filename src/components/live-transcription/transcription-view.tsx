...
export function TranscriptionView({
  ...
  const [viewMode, setViewMode] = useState<"overlay" | "sidebar" | "timestamp">(
    "overlay"
  );
  ...
  const formatSpeaker = (speaker: string | undefined) => {
    ...
    return speaker.startsWith("speaker_")
                  ...
                  {viewMode === "overlay" ? (
                    <>
                      <ChunkOverlay
                        ...
                        displaySpeaker={
                          chunk.speaker
                            ? getDisplaySpeaker(chunk.speaker)
                            : "speaker_0"
                        }
                        onSpeakerClick={() => {
                          if (chunk.speaker) {
                      ...
                      <div className="relative">
                        ...
                        >
                          {data?.editedMergedChunks[chunk.id]?.diffs ? (
                            <DiffText
                              diffs={data.editedMergedChunks[chunk.id].diffs}
                            />
                          ) : (
                            data?.editedMergedChunks[chunk.id]?.text ||
                            chunk.text
                          )}
                        </div>
                      </div>
      ...
      <Dialog open={mergeModalOpen} onOpenChange={setMergeModalOpen}>
        <DialogContent className="sm:max-w-md max-h-[70vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>
              Rename or merge{" "}
              {formatSpeaker(getDisplaySpeaker(selectedSpeaker!))}
            </DialogTitle>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <button
                onClick={() =>
                  mergeSpeakers(getDisplaySpeaker(selectedSpeaker!))
                }
