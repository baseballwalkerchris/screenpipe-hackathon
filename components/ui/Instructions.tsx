...
export function Instructions({ 
  ...
  return (
    <div className={`${taskStarted ? 'bg-white rounded-lg shadow-lg w-[320px]' : 'max-w-2xl'}`}>
      <div className={`${taskStarted ? 'p-4' : ''}`}>
        <div className={`space-y-${taskStarted ? '2' : '5'}`}>
          ...
          <h1 className={`${taskStarted ? 'text-base' : 'text-3xl'} font-semibold`}>
            {title}
          </h1>
      ...
      <div className={taskStarted ? 'mt-4 bg-gray-50 p-3 rounded-b-lg border-t' : 'mt-7'}>
        {!taskStarted ? (
          <Button 
            onClick={onStartTask}
