...
export function PlaygroundCard({
  ...
  const formatRawOutput = (
    ...
    if (error) {
      return `> ${
        endpoint?.startsWith("SDK:")
          ? endpoint
          : `${method} ${endpoint || "[No endpoint specified]"}`
      }
> Error: ${error}`;
    }
...
3. Return to this "raw output" tab to see the results`);
  ...
  const llmContext =
    apiDocs ||
    `Loading reference documentation... See docs at ${llmContextUrl}`;
        ...
        <CardContent className="p-2">
          <Tabs defaultValue="rendered" orientation="vertical" className="flex">
            <TabsList className="mr-4 h-auto flex-col space-y-1 bg-slate-200 dark:bg-slate-800 p-2 rounded-md self-start w-40">
              <TabsTrigger
                value="rendered"
                className="justify-start font-mono text-xs w-full text-left"
              >
                rendered output
              </TabsTrigger>
              <TabsTrigger
            ...
            <div className="flex-1 relative">
              <TabsContent
                value="prompt"
                className="p-3 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md mt-0 absolute inset-0 overflow-auto h-[350px]"
              >
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    >
                      {copiedButton === "entire-prompt" ? (
                        ...
                        <>
                          <Copy className="h-3.5 w-3.5 mr-1" /> copy entire
                          prompt
                        </>
                      )}
                    </Button>
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <h3 className="text-sm text-gray-500 font-normal">
                        ...
                        <a
                          ...
                          className="text-xs text-blue-500 hover:underline inline"
                        >
                          {llmContextUrl}
                        </a>
                        ):
              ...
              >
                {rawOutput ? (
                  ...
                  >
                    <p
                      className="text-sm text-slate-500 font-mono text-center mt-4"
                      onClick={(e) => {
